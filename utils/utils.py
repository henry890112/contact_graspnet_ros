...
def load_data(object_name, multiview_index):
    '''
    return type: numpy array(rgb, depth, mask)
    '''
    rgb_filename = f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/color{multiview_index}_{object_name}.png'
    depth_filename = f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/depth{multiview_index}_{object_name}.npy'
    mask_filename = f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/mask{multiview_index}_{object_name}.png'
...
def save_observation_images(env, placed_obj, multiview_index=0):
    """
    ...
    for i, index in enumerate(placed_obj.keys()):
        ...
        cv2.imwrite(f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/color{multiview_index}_{placed_name}.png', rgb*255)

        # 保存深度圖像
        depth = obs[1][3].T
        np.save(f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/depth{multiview_index}_{placed_name}.npy', depth)  # 單位為公尺

        # 保存掩碼圖像
        mask = obs[1][4].T
        ones = np.ones(shape=(640, 640))
        mask = ones - mask
        cv2.imwrite(f'/home/user/contact_graspnet_ws/src/contact-graspnet-ros/multiview_data/mask{multiview_index}_{placed_name}.png', mask)
